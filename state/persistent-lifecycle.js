...
class PersistentLifecycle {
  ...
  transition(lifecycleId, to, data = {}) {
    ...
    if (to === 'RESIMULATED' && data.transactionHash !== current.transactionHash) throw new Error('RESIMULATION_BINDING_MISMATCH');
    if (to === 'APPROVED' && data.transactionHash !== current.transactionHash) throw new Error('APPROVAL_BINDING_MISMATCH');
  ...
  timelock(lifecycleId, transactionHash) {
    ...
    if (!transactionHash || transactionHash !== current.transactionHash) throw new Error('TRANSACTION_BINDING_MISMATCH');
