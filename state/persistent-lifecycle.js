...
const ALLOWED = Object.freeze({
  ...
  BROADCAST: ['VERIFIED', 'REJECTED'],
  REJECTED: [], EXPIRED: [], VERIFIED: [],
});
...
class PersistentLifecycle {
  constructor({ filePath, timelockSeconds = 172800, now = () => Math.floor(Date.now() / 1000) }) {
    this.audit = new AuditLog(filePath);
  ...
  _recover() {
    for (const event of this.audit.records()) {
      ...
      if (event.sequence < 0 || event.data.version !== expectedVersion) throw new Error('LIFECYCLE_VERSION_BROKEN');
  ...
  create({ lifecycleId, proposal, proposalHash }) {
    ...
    return this._transition(lifecycleId, 'CREATED', 'CREATED', { proposalHash });
  }
  ...
  transition(lifecycleId, to, data = {}) {
    ...
    if (to === 'APPROVED' && current.state === 'SIMULATED' && current.unlockAt && current.unlockAt > this.now()) {
      throw new Error('TIMELOCK_ACTIVE');
    }
    if (to === 'RESIMULATION_REQUIRED' && current.state === 'TIMELOCKED' && current.unlockAt > this.now()) {
      throw new Error('TIMELOCK_ACTIVE');
    }
    if (to === 'APPROVED' && current.state === 'RESIMULATED' && data.transactionHash !== current.transactionHash) {
      throw new Error('RESIMULATION_BINDING_MISMATCH');
    }
  ...
  timelock(lifecycleId, transactionHash) {
    ...
    const unlockAt = this.now() + this.timelockSeconds;
    return this._transition(lifecycleId, current.state, 'TIMELOCKED', { transactionHash, unlockAt });
  }
  ...
  _transition(id, from, to, data) {
    ...
    const event = this.audit.append({
      lifecycleId: id, from, to, event: `STATE_${to}`,
      data: { ...structuredClone(data), version, proposalHash: data.proposalHash ?? current?.proposalHash, transactionHash: data.transactionHash ?? current?.transactionHash, unlockAt: data.unlockAt ?? current?.unlockAt },
    });
    this._state.set(id, {
      version, state: to, proposalHash: event.data.proposalHash,
      transactionHash: event.data.transactionHash, unlockAt: event.data.unlockAt,
    });
    return this.get(id);
  }
}

module.exports = { PersistentLifecycle, ALLOWED };
