...
const STATES = Object.freeze([
  'CREATED', 'HASHED', 'BUILT', 'SIMULATED', 'TIMELOCKED',
  'RE_SIMULATED', 'APPROVED', 'SIGNED', 'BROADCAST', 'CONFIRMED', 'VERIFIED', 'REJECTED'
]);

const TRANSITIONS = Object.freeze({
  CREATED: ['HASHED', 'REJECTED'],
  HASHED: ['BUILT', 'REJECTED'],
  BUILT: ['SIMULATED', 'REJECTED'],
  SIMULATED: ['TIMELOCKED', 'REJECTED'],
  TIMELOCKED: ['RE_SIMULATED', 'REJECTED'],
  RE_SIMULATED: ['APPROVED', 'REJECTED'],
  APPROVED: ['SIGNED', 'REJECTED'],
  SIGNED: ['BROADCAST', 'REJECTED'],
...
class PersistentLifecycle {
  ...
  createProposal(proposal) {
    ...
    const record = {
      ...
      state: 'CREATED',
      version: 0,
      transactionBindingHash: null,
      timelockUntil: null,
      simulationBindingHash: null,
      approval: null
    };
    this._transition(record, 'CREATED', { proposalHash: proposal.proposalHash });
  ...
  transition(proposalId, nextState, payload = {}, expectedVersion) {
    ...
    this._transition(record, nextState, payload);
  ...
  bindTransaction(proposalId, transactionBindingHash) {
    ...
    this._transition(record, 'BUILT', { transactionBindingHash });
    return this.get(proposalId);
  }
  ...
  recordSimulation(proposalId, transactionBindingHash, { success, phase = 'initial' } = {}) {
    ...
    if (phase === 'initial') {
      this._transition(record, 'SIMULATED', { transactionBindingHash });
  ...
  startTimelock(proposalId, durationMs = 172800000) {
    ...
    this._transition(record, 'TIMELOCKED', { timelockUntil: record.timelockUntil });
    return this.get(proposalId);
  }
  ...
  approve(proposalId, approvalId) {
    ...
    if (this.clock() < record.timelockUntil) throw new Error('TIMELOCK_ACTIVE');
    record.approval = Object.freeze({ approvalId, proposalHash: record.proposal.proposalHash, transactionBindingHash: record.transactionBindingHash });
    this._transition(record, 'APPROVED', { approvalId, transactionBindingHash: record.transactionBindingHash });
    return this.get(proposalId);
  }
  ...
  verifyAuditChain() {
    ...
    for (const line of lines) {
      ...
      if (event.previousHash !== previous) throw new Error('AUDIT_CHAIN_BROKEN');
      const { eventHash, ...unsigned } = event;
      if (hash(unsigned) !== eventHash) throw new Error('AUDIT_EVENT_TAMPERED');
      previous = eventHash;
    }
    return true;
  }
  ...
  _transition(record, nextState, payload) {
    if (record.state !== nextState && !(TRANSITIONS[record.state] || []).includes(nextState)) {
    ...
    record.version += 1;
    this._append({
      type: 'STATE_TRANSITION', proposalId: record.proposal.proposalId,
      proposalHash: record.proposal.proposalHash, previousState, nextState,
      version: record.version, payload
    });
  }

  _append(data) {
    const unsigned = { timestamp: this.clock(), previousHash: this.lastAuditHash, ...data };
    const event = { ...unsigned, eventHash: hash(unsigned) };
    fs.appendFileSync(this.filePath, `${JSON.stringify(event)}\n`, { encoding: 'utf8' });
    const fd = fs.openSync(this.filePath, 'r');
    try { fs.fsyncSync(fd); } finally { fs.closeSync(fd); }
    this.lastAuditHash = event.eventHash;
  }
  ...
  _recover() {
    ...
    for (const line of lines) {
      ...
      if (event.type === 'STATE_TRANSITION') {
        if (!record) {
          record = { proposal: { proposalId: event.proposalId, proposalHash: event.proposalHash }, state: event.nextState,
            version: event.version, transactionBindingHash: null, timelockUntil: null, simulationBindingHash: null, approval: null };
          this.records.set(event.proposalId, record);
        } else {
          record.state = event.nextState;
          record.version = event.version;
        }
        const p = event.payload || {};
        if (p.transactionBindingHash) record.transactionBindingHash = p.transactionBindingHash;
        if (p.timelockUntil) record.timelockUntil = p.timelockUntil;
        if (p.approvalId) record.approval = { approvalId: p.approvalId, proposalHash: event.proposalHash, transactionBindingHash: p.transactionBindingHash };
      }
    }
