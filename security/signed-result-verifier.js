...
function loadSeenResultIds(resultJournalPath){
  ...
  if(!fs.existsSync(resultJournalPath))return new Set();
  const seen=new Set();
  for(const line of fs.readFileSync(resultJournalPath,'utf8').split('\n').filter(Boolean)){
    const e=JSON.parse(line);
    if(typeof e.resultId!=='string'||!e.resultId)throw new Error('RESULT_JOURNAL_INVALID');
    seen.add(e.resultId);
  }
  return seen;
}
...
function persistResultId(resultJournalPath,resultId){
  ...
  const fd=fs.openSync(resultJournalPath,'a',{mode:0o600});
  try{
    fs.writeSync(fd,`${JSON.stringify({resultId})}\n`);
    fs.fsyncSync(fd);
...
function verifySignedResult({result,publicKey,lifecycle,now=Date.now,maxAgeMs=300000,seenResultIds,resultJournalPath}={}){
  ...
  const seen=seenResultIds||loadSeenResultIds(resultJournalPath);
  if(seen.has(result.resultId))throw new Error('RESULT_DUPLICATE');
  let valid=false;
  ...
  persistResultId(resultJournalPath,result.resultId);
  if(seenResultIds)seenResultIds.add(result.resultId);
